...
const AlarmDetailPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchAlarmDetails = async () => {
            try {
                ...
                setNotificationMessage(err.response && err.response.status === 404 ? 'Alarm not found.' : 'Failed to load alarm details.');
                setNotificationType('error');
            }
        };
    ...
    useEffect(() => {
        if (location.state?.notifyFailed) {
            setNotificationMessage('Notification failed. Call the guard and confirm manual handling.');
            setNotificationType('error');
    ...
    const notifyGuard = async (guardID) => {
        ...
        try {
            const response = await axios.post(`http://127.0.0.1:5000/alarms/notify/${guardID}/${id}`, {}, {
                headers: {
            ...
            console.error('Error notifying the guard:', err.response ? err.response.data : err.message);
    ...
    const handleNotifyAndUpdate = async () => {
        ...
        const confirmNotify = window.confirm('Are you sure you want to notify the guard?');
        if (!confirmNotify) {
            return;
        }
        ...
        try {
            ...
            if (notifySuccess) {
                ...
                setNotificationMessage('Notification failed. Call the guard immediately to ensure the alert is acknowledged.');
                setNotificationType('error');
            }
        } catch (err) {
    ...
    const handleDismissAlert = () => {
        const confirmDismiss = window.confirm('Are you sure you want to dismiss the alarm?');
        if (!confirmDismiss) {
            return;
        }
    ...
    const handleManualNotifyConfirm = async () => {
        if (callChecked) {
            await updateAlarmStatus('notified');
            setNotificationMessage('Manual notification confirmed. Status updated to notified.');
            setNotificationType('success');
            setManualNotifyVisible(false);
        } else {
            setNotificationMessage('Please call the guard and check Call to confirm manual notification.');
            setNotificationType('error');
        }
    };
            ...
            <div className="flex-grow flex flex-col items-center justify-center p-8">
                    ...
                    <div className="w-2/5 bg-gray-200 rounded-lg p-2 ml-2">
                        {alarm ? (
                            <>
                                ...
                                <p className="text-lg">
                                    Confidence Level: {alarm.confidence_score ? (alarm.confidence_score * 100).toFixed(2) + '%' : 'N/A'}
                                </p>
                                <p className="text-lg">
                                    Timestamp: {alarm.timestamp !== 'N/A' ? new Date(alarm.timestamp).toLocaleString() : 'N/A'}
                                </p>
                ...
                <div className="flex flex-col items-center w-10/12 max-w-6xl mt-6">
                    ...
                    <div className="mt-2 h-6 flex flex-col items-center ">
                        {notificationMessage && (
                            <p className={`text-center ${notificationType === 'success' ? 'text-green-500' : 'text-red-500'}`}>
                                {notificationMessage}
                            </p>
                        )}
                        {manualNotifyVisible && (
