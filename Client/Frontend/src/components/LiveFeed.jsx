...
const LiveFeed = () => {
    ...
    const updateAlarmStatus = async (newStatus) => {
        const confirmAction = window.confirm(`Are you sure you want to ${newStatus === 'ignored' ? 'dismiss' : 'update'} the alarm?`);
    ...
    const notifyGuard = async () => {
        ...
        try {
            ...
            setNotificationMessage('Notification failed. Please navigate back to handle the alert manually.');
            setNotificationType('error');
        }
    };
    ...
    return (
        <div className="flex flex-col items-center justify-center flex-grow p-4">
            <h2 className="text-xl font-semibold mb-2 text-[#2E5984]">Live feed of Camera : 2</h2>
            ...
            <div className="mt-3 text-sm h-6">
                {notificationMessage && (
                    <p className={`${notificationType === 'success' ? 'text-green-500' : 'text-red-500'}`}>
                        {notificationMessage}
                    </p>
                )}
            </div>
