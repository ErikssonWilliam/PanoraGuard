function AlertsChart() {
  ...
  return (
    <>
      <img
        loading="lazy"
        src="https://cdn.builder.io/api/v1/image/assets/TEMP/2b2848b5dc6b0f8e3833af828d1959c0f5a29bcf6c9b7dd2e5b672b3ee07dd15?apiKey=b098bcbfae4a413cac7bdf6cae526d40&"
        alt="Alerts chart"
        className="object-contain self-stretch mt-24 ml-7 w-full aspect-[1000] max-w-[881px] max-md:mt-10 max-md:max-w-full"
      />
      <div className="flex flex-wrap gap-5 justify-between items-start self-stretch mt-2.5 mr-3.5 w-full max-md:mr-2.5 max-md:max-w-full">
        {chartData.map((item) => (
          <div
            key={item.day}
            className="flex gap-2 mt-5 text-xs tracking-wide leading-none text-center text-gray-500 whitespace-nowrap"
          >
            <div className="my-auto">{item.day}</div>
            <div
              className="flex shrink-0 bg-sky-900 h-[90px] w-[11px]"
              style={{ height: `${item.truePositive}px` }}
            />
            <div className="flex gap-1 items-start self-end mt-9">
              <div
                className="flex shrink-0 mt-1 w-2.5 bg-gray-200 h-[50px]"
                style={{ height: `${item.falsePositive}px` }}
              />
              <div>{item.day}</div>
            </div>
          </div>
      ...
      <img
        loading="lazy"
        src="https://cdn.builder.io/api/v1/image/assets/TEMP/1dcd7324415fb36a8cbc39244a343eff111de0bf6a44e3201c94492696dc2a6a?apiKey=b098bcbfae4a413cac7bdf6cae526d40&"
        alt="Alerts chart legend"
        className="object-contain self-stretch ml-7 w-full aspect-[1000] max-w-[881px] max-md:max-w-full"
      />
