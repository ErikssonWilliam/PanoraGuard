...
// Function to enable the best snapshot feature by sending an HTTP request
static void enable_best_snapshot() {
    const char* url = "http://<camera-ip>/config/rest/best-snapshot/v1/enabled"; // Replace with your camera IP
    const char* data = "{\"data\":true}"; // JSON payload to enable best snapshot
    send_http_request(url, data);
}

// Function to retrieve the best snapshot from the camera
static void retrieve_best_snapshot(char* snapshot_data, size_t snapshot_data_size) {
    const char* url = "http://<camera-ip>/config/rest/best-snapshot/v1/snapshot"; // Replace with your camera IP
    CURL *curl;
    CURLcode res;
    curl = curl_easy_init();
    if(curl) {
...
// Function to send best snapshot and metadata to the external server
static void send_snapshot_and_metadata(const char* metadata, const char* snapshot_data) {
    const char* url = "http://192.168.1.123:5000/camera/data"; // Change this to your server's endpoint
    ...
    if(curl) {
        char post_data[1024];
        snprintf(post_data, sizeof(post_data), "%s&snapshot_data=%s", metadata, snapshot_data);
        ...
        res = curl_easy_perform(curl);
        if(res != CURLE_OK)
            syslog(LOG_ERR, "Failed to send data: %s", curl_easy_strerror(res));
...
static void on_message(const mdb_message_t* message, void* user_data) {
    ...
    if (parse_json_payload(payload, &type_value, &score_value)) {
        ...
        char snapshot_data[65536]; // Buffer to hold the snapshot data
        retrieve_best_snapshot(snapshot_data, sizeof(snapshot_data));

        send_snapshot_and_metadata(metadata_str, snapshot_data);
